...
plane_img = pygame.image.load('assets/images/plane.png')
logo_img = pygame.image.load('assets/images/logo.png')
fighter_img = pygame.image.load('assets/images/fighter.png')
clouds_img = pygame.image.load('assets/images/clouds.png')
clouds_img = pygame.transform.scale(clouds_img, (WIDTH, 350))

home_img = pygame.image.load('assets/images/buttons/homeBtn.png')
replay_img = pygame.image.load('assets/images/buttons/replay.png')
sound_off_img = pygame.image.load("assets/images/buttons/soundOffBtn.png")
sound_on_img = pygame.image.load("assets/images/buttons/soundOnBtn.png")
...
tap_to_play_font = 'assets/fonts/BubblegumSans-Regular.ttf'
...
player_bullet_fx = pygame.mixer.Sound('assets/audio/gunshot.wav')
click_fx = pygame.mixer.Sound('assets/audio/click.mp3')
collision_fx = pygame.mixer.Sound('assets/audio/mini_exp.mp3')
blast_fx = pygame.mixer.Sound('assets/audio/blast.wav')
fuel_fx = pygame.mixer.Sound('assets/audio/fuel.wav')

pygame.mixer.music.load('assets/audio/Defrini - Spookie.mp3')
...
# -------------------- CHEAT --------------------
invincible = False  # cheat mati di awal
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if event.key == pygame.K_c:
                ...
                print(f"Cheat mode {'ON' if invincible else 'OFF'}")
        ...
        if p.alive:
            ...
            for bullet in player_bullet_group:
                ...
                for plane in planes_hit:
                    plane.health -= bullet.damage if not invincible else 1000
                    if plane.health <= 0:
            ...
            # tabrakan player dengan musuh
            player_collide = pygame.sprite.spritecollide(p, enemy_group, True)
            if player_collide and not invincible:
                x, y = p.rect.center
                explosion_obj = explosion(x, y, 2)
                explosion_group.add(explosion_obj)
            ...
            if invincible:
                p.health = 100
                p.fuel = 100
                p.powerup = 99  # banyak powerup

        # -------------------- GAME OVER --------------------
        if (not p.alive or p.fuel <= -10) and not invincible:
