...
class Player:
	def __init__(self, x, y):
		...
		self.image_list = []
		for i in range(2):
			img = pygame.image.load(f'assets/images/player{i+1}.png')
...
class Enemy(pygame.sprite.Sprite):
	def __init__(self, x, y, type_):
		...
		for i in range(2):
			if type_ == 1:
				img = pygame.image.load(f'assets/images/enemies/enemy1-{i+1}.png')
			if type_ == 2:
				img = pygame.image.load(f'assets/images/enemies/enemy2-{i+1}.png')
			if type_ == 3:
				img = pygame.image.load(f'assets/images/enemies/enemy3-{i+1}.png')
			if type_ == 4:
				img = pygame.image.load(f'assets/images/choppers/chopper1-{i+1}.png')
			if type_ == 5:
				img = pygame.image.load(f'assets/images/choppers/chopper2-{i+1}.png')
		...
		if self.type in (1,2,3):
			self.fx = pygame.mixer.Sound('assets/audio/plane.mp3')
		elif self.type in (4, 5):
			self.fx = pygame.mixer.Sound('assets/audio/chopper.mp3')
...
class Bullet(pygame.sprite.Sprite):
	def __init__(self, x, y, type_, dx=None):
		...
		if type_ == 1:
			self.image = pygame.image.load('assets/images/bullets/1.png')
			self.image = pygame.transform.scale(self.image, (20, 40))
		if type_ == 2:
			self.image = pygame.image.load('assets/images/bullets/2.png')
			self.image = pygame.transform.scale(self.image, (15, 30))
		if type_ == 3:
			self.image = pygame.image.load('assets/images/bullets/3.png')
			self.image = pygame.transform.scale(self.image, (20, 40))
		if type_ in (4, 5):
			self.image = pygame.image.load('assets/images/bullets/4.png')
			self.image = pygame.transform.scale(self.image, (20, 20))
		if type_ == 6:
			self.image = pygame.image.load('assets/images/bullets/red_fire.png')
			self.image = pygame.transform.scale(self.image, (15, 30))
...
class explosion(pygame.sprite.Sprite):
	def __init__(self, x, y, type_):
		...
		for i in range(self.length):
			img = pygame.image.load(f'assets/images/explosion{type_}/{i+1}.png')
			w, h = img.get_size()
			width = int(w * 0.40)
			height = int(w * 0.40)
...
class Fuel(pygame.sprite.Sprite):
	def __init__(self, x, y):
		...
		self.image = pygame.image.load('assets/images/fuel.png')
		self.rect = self.image.get_rect(center=(x, y))
